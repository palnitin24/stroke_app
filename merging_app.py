...
def stroke_predict(input_parameters: ModelInput):
    input_data = input_parameters.json()
    input_dictionary = json.loads(input_data)

    gen = map_categorical(input_parameters.Gender, {'Male': 1, 'Female': 0})
    work = map_categorical(input_parameters.Work_type, {'Child': 0, 'Government': 1, 'Never worked': 2, 'Private': 3, 'Self-employed': 4})
    hyp = map_categorical(input_parameters.Hypertension, {'No': 0, 'Yes': 1})
    heart = map_categorical(input_parameters.Heart_disease, {'No': 0, 'Yes': 1})
    married = map_categorical(input_parameters.ever_Married, {'No': 0, 'Yes': 1})
    residence = map_categorical(input_parameters.Residence_type, {'Urban': 1, 'Rural': 0})
    glucose_level = input_parameters.Avg_Glucose_Level
    bmi = input_parameters.BMI
    age = input_parameters.AGE
    smoking = map_categorical(input_parameters.Smoking_status, {'Never Smoked': 0, 'Formerly Smoked': 1, 'Smokes': 2, 'Unknown': 3})

    input_list = [gen, age, hyp, heart, married, work, residence,
                                glucose_level, bmi, smoking]
    input_array = np.array(input_list).reshape(1, -1)
    normalized_input_data = scaler.transform(input_array)
    
    prediction = loaded_model.predict(normalized_input_data)
    confidence = loaded_model.predict_proba(normalized_input_data)[:, 1]
    ...
    if updated_res == 0:
        ...
        return {'result': 'The person may be at risk of a Stroke'}
